import { ensure, invariant } from "@code-ui/utils";
import type {
  Machine,
  MachineSchema,
  MachineState,
...
function stripAbsolutePrefix(value: string) {
  return isExplicitAbsoluteStatePath(value)
    ? value.slice(ABSOLUTE_PREFIX.length)
    : value;
}
...
function buildStateIndex<T extends MachineSchema>(machine: Machine<T>) {
  ...
  const visit = (basePath: string, state: MachineState<T>) => {
    ...
    if (!childStates) return;
      ...
      state.initial,
      () =>
        `[zag-js] Compound state "${basePath}" has child states but no "initial" property`,
    );
...
function getStatePathById<T extends MachineSchema>(
  machine: Machine<T>,
  stateId: string,
) {
...
function resolveAbsoluteStateValue<T extends MachineSchema>(
  machine: Machine<T>,
  value: string,
): T["state"] {
  const stateIndex = ensureStateIndex(machine);
...
function hasStatePath<T extends MachineSchema>(
  machine: Machine<T>,
  value: string,
) {
  const stateIndex = ensureStateIndex(machine);
...
export function resolveStateValue<T extends MachineSchema>(
  ...
  if (!isAbsoluteStatePath(stateValue) && source) {
    ...
    for (let index = sourceSegments.length - 1; index >= 1; index--) {
      ...
      if (hasStatePath(machine, candidate))
        return resolveAbsoluteStateValue(machine, candidate);
    }

    // Check root-level siblings
    if (hasStatePath(machine, stateValue))
...
export function getStateDefinition<T extends MachineSchema>(
  machine: Machine<T>,
  state: T["state"],
) {
...
export function findTransition<T extends MachineSchema>(
  ...
  for (let index = chain.length - 1; index >= 0; index--) {
    const transitionMap = chain[index]?.state.on as
      | TransitionMap<T>
      | undefined;
    const transition = transitionMap?.[eventType];
    if (transition)
      return { transitions: transition, source: chain[index]?.path };
  }
...
export function getExitEnterStates<T extends MachineSchema>(
  ...
  const sameLeaf =
    prevChain[prevChain.length - 1]?.path ===
    nextChain[nextChain.length - 1]?.path;
...
export function hasTag<T extends MachineSchema>(
  machine: Machine<T>,
  state: T["state"],
  tag: T["tag"],
) {
  return getStateChain(machine, state).some((item) =>
    item.state.tags?.includes(tag),
  );
}
