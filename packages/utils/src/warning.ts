...
export function warn(...a: unknown[]): void {
  const m = (a.length === 1 ? a[0] : a[1]) as string;
  const c = (a.length === 2 ? a[0] : true) as boolean;
...
export function invariant(...a: unknown[]): void {
  const m = (a.length === 1 ? a[0] : a[1]) as string;
  const c = (a.length === 2 ? a[0] : true) as boolean;
  if (c && process.env.NODE_ENV !== "production") {
    throw new Error(m);
...
export function ensure<T>(
  ...
  if (c === null || c === undefined) throw new Error(m());
}
...
export function ensureProps<T, K extends keyof T>(
  ...
  for (const key of keys) {
    if (props[key] === null || props[key] === undefined) missingKeys.push(key);
  }
  if (missingKeys.length > 0)
    throw new Error(
      `[code-ui${scope ? ` > ${scope}` : ""}] missing required props: ${missingKeys.join(", ")}`,
    );
}
